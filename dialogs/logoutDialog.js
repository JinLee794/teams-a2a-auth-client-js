    ...
    async interrupt(innerDc) {
        if (innerDc.context.activity.type === ActivityTypes.Message) {
            ...
            if (text === 'logout') {
                ...
                // Sign out the user and invalidate all tokens
                await userTokenClient.signOutUser(activity.from.id, this.connectionName, activity.channelId);
                
                // Additional token cleanup - get and revoke any existing token
                try {
                    const token = await userTokenClient.getUserToken(activity.from.id, this.connectionName, activity.channelId);
                    if (token) {
                        // Force token revocation to clear cache
                        await userTokenClient.signOutUser(activity.from.id, this.connectionName, activity.channelId);
                    }
                } catch (error) {
                    // Token might already be invalid, continue with logout
                    console.log('Token cleanup error (expected if already signed out):', error.message);
                }
